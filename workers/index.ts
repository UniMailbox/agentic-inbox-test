...
app.put("/api/v1/admin/users/:userId/role", requireAdmin, async (c) => {
	...
	try {
		const updated = await setUserRole(c.env, userId, body.role, c.var.user.email);
...
app.delete("/api/v1/admin/users/:userId", requireAdmin, async (c) => {
	const userId = decodeURIComponent(c.req.param("userId") ?? "");
	try {
		const updated = await deactivateUser(c.env, userId, c.var.user.email);
		return c.json(updated);
	} catch (e) {
		const msg = (e as Error).message;
